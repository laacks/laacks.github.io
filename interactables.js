...
AFRAME.registerComponent('sword-in-stone', {
  ...
  init: function () {
    ...
    // Helper to force all meshes in model opaque
    this._fixMaterials = () => {
  ...
  tick: function (t, dt) {
    ...
    if (!this.held && !this.dropped) {
      ...
      if (trig && !this.triggerDown && sp.distanceTo(cp) < this.data.grabDist) {
        ...
        // Re-apply opacity fix — GLB materials reset when object moves
      ...
      // Offset sword down so handle sits in palm rather than floating above
      // 0.08m = roughly 3 inches down from controller center
