...
AFRAME.registerComponent('sword-in-stone', {
  ...
  init: function () {
    ...
    sword.setAttribute('rotation', this.data.rotOffset);

    // Pulse glow animation on the model itself
  ...
  tick: function (t, dt) {
    ...
    if (!this.held && !this.dropped) {
      ...
      // Apply rotation offset so sword aligns with controller
      const rotOff = this.data.rotOffset.split(' ').map(Number);
      const offset = new THREE.Quaternion().setFromEuler(
        new THREE.Euler(
          THREE.MathUtils.degToRad(rotOff[0]),
          THREE.MathUtils.degToRad(rotOff[1]),
          THREE.MathUtils.degToRad(rotOff[2])
        )
      );
      const swordQuat = cr.clone().multiply(offset);

      // Blade tip direction (along controller forward after offset)
